...
<head>
    ...
    <title>{{ $fr ? 'Votre commande a été annulée' : 'Your order has been cancelled' }}</title>
</head>
                    ...
                    <tr>
                        <td style="padding:48px 40px 0;text-align:center;">
                            ...
                            <h1 style="margin:0 0 12px;font-size:22px;font-weight:400;color:#1A1A1A;letter-spacing:-0.02em;">
                                {{ $fr ? 'Votre commande a été annulée' : 'Your order has been cancelled' }}
                            </h1>
                            <p style="margin:0;font-size:14px;color:#9A9490;font-weight:300;line-height:1.6;">
                                ...
                                {{ $fr ? 'a été annulée.' : 'has been cancelled.' }}
                            </p>
                        </td>
                    </tr>
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td style="background:#F5F2EE;border:1px solid #E8E2D9;border-left:3px solid #C8A96E;padding:20px 24px;">
                                        ...
                                        <p style="margin:0;font-size:14px;color:#1A1A1A;font-weight:400;line-height:1.5;">
                                            @if($fr)
                                                Un remboursement complet de <strong>{{ number_format($order->total, 2) }} CHF</strong> a été émis sur votre moyen de paiement d'origine.<br>
                                                <span style="color:#9A9490;font-size:12px;">Comptez 5 à 10 jours ouvrés pour qu'il apparaisse sur votre compte.</span>
                                            @else
                                                A full refund of <strong>{{ number_format($order->total, 2) }} CHF</strong> has been issued to your original payment method.<br>
                                                <span style="color:#9A9490;font-size:12px;">Please allow 5 to 10 business days for it to appear.</span>
                                            @endif
                                        </p>
                                    </td>
                                </tr>
                    ...
                    <tr>
                        <td style="padding:32px 40px 0;">
                            <p style="margin:0 0 16px;font-size:11px;letter-spacing:0.15em;text-transform:uppercase;color:#9A9490;">
                                {{ $fr ? 'Articles annulés' : 'Cancelled items' }}
                            </p>
                    ...
                    <tr>
                        <td style="padding:32px 40px;">
                            <p style="margin:0;font-size:14px;color:#6B6560;font-weight:300;line-height:1.7;">
                                {{ $fr
                                    ? 'Nous espérons avoir l\'occasion de vous servir à nouveau. Si vous avez des questions concernant cette annulation, n\'hésitez pas à nous contacter.'
                                    : 'We hope to serve you again in the future. If you have any questions about your cancellation, feel free to contact us.' }}
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding:24px 40px;text-align:center;">
                            <p style="margin:0 0 6px;font-size:11px;color:#9A9490;font-weight:300;letter-spacing:0.05em;">
                                {{ $fr ? 'Merci pour votre compréhension.' : 'Thank you for your understanding.' }}
                            </p>
